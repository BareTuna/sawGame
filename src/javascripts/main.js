...
let letChoose = false
let menuButtons = [
    new Button(200, 200, 150, 25, "Time trial", () => {
        scene = TIMETRIAL
        ttTimer = 0
        score = 0
        player.hasSaw = true
    ...
    new Button(200, 230, 150, 25, "Levels", () => (scene = LEVELS)),
    new Button(200, 260, 150, 25, "Survival", () => {
        scene = SURVIVE
        scene = SURVIVE
        score = 0
        player.hasSaw = true
...
let ttOverButtons = [
    new Button(200, 300, 150, 25, "Try Again? (space)", () => (scene = TIMETRIAL)),
    new Button(200, 335, 100, 25, "Menu", () => (scene = MENU)),
]
let levelButtons = [new Button(200, 300, 150, 25, "Menu", () => (scene = MENU))]
let surviveOverButtons = [
    new Button(200, 300, 150, 25, "Try Again? (space)", () => {
        scene = SURVIVE
        score = 0
        player.hasSaw = true
        player.x = 200
...
function surviveDraw() {
    ...
    if (ttTimer <= 0) {
        ...
        if (
            breatheTimer <= 0 &&
            letChoose == false &&
            (potentialPointCount + score < stageBreaks[stage] ||
                stage == COLUMNSTAGE ||
                stage == BULLETSTAGE ||
                stage == DREVILSTAGE)
        ) {
    ...
    if (
        breatheTimer > 0 &&
        enemies.length == 0 &&
        (score >= stageBreaks[stage] || stage == COLUMNSTAGE || stage == BULLETSTAGE || stage == DREVIL)
    ) {
        push()
